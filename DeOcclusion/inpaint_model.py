...
def build_graph_with_loss_two(gt_bath_data, batch_data, batch_size, mask, vgg_path, adv_type, stage=0,
                          lambda_style=0.001, lambda_cor=0.00001, alpha=0.01, lambda_adv=1.0,
                          reuse=False, training=True):
    image_gt = tf.subtract(tf.divide(gt_bath_data, 127.5), 1.)
    image_data = tf.subtract(tf.divide(batch_data, 127.5), 1.)
    date_shape = batch_data.get_shape().as_list()
    batch_incomplete = image_data * mask
    ...
    if stage == 1:
        ...
        if adv_type == 'wgan_gp':
            ...
            d_loss = tf.reduce_mean(d_pred * mask_label) + tf.reduce_mean(d_pred2 * mask_label) - 0.01 * tf.reduce_mean(
                d_real * mask_label) + penalty_loss
            d_g_loss = -1 * tf.reduce_mean(d_pred * mask_label) - tf.reduce_mean(d_pred2 * mask_label)

        elif adv_type == 'gan':
            adv_d_loss = 0.01 * tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=d_real, labels=tf.ones_like(d_real)) * mask_label)
            + tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=d_pred, labels=tf.zeros_like(d_pred)) * mask_label)
            + tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=d_pred2, labels=tf.zeros_like(d_pred2)) * mask_label)
            adv_g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=d_pred, labels=tf.ones_like(
                d_real)) * mask_label) + tf.reduce_mean(
                tf.nn.sigmoid_cross_entropy_with_logits(logits=d_pred2, labels=tf.ones_like(d_real)) * mask_label)

        elif adv_type == 'hinge':

            adv_d_loss = 0.01 * tf.reduce_mean(tf.nn.relu(1 - d_real) * mask_label) + tf.reduce_mean(
                tf.nn.relu(1 + d_pred) * mask_label)
            + tf.reduce_mean(tf.nn.relu(1 + d_pred2) * mask_label)
            adv_g_loss = -1 * tf.reduce_mean(tf.nn.relu(d_pred) * mask_label) - tf.reduce_mean(
                tf.nn.relu(d_pred2) * mask_label)
        else:

            adv_d_loss = alpha * tf.reduce_sum(tf.abs(mask_label - d_real)) + tf.reduce_sum(
                tf.abs(0 - d_pred)) + tf.reduce_sum(tf.abs(0 - d_pred2))
    ...
    tf.summary.image('incomplete', batch_incomplete, max_outputs=4)
    tf.summary.image('image_p1', image_p1, max_outputs=4)
    tf.summary.image('image_p2', image_p2, max_outputs=4)
    tf.summary.image('image_c2', image_c2, max_outputs=4)
