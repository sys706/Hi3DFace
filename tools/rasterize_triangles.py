...
rasterize_triangles_module = tf.load_op_library(
    'tools/kernels/rasterize_triangles_kernel.so')
...
def rasterize_clip_space(vertices, attributes, triangles,
  ...
  uncorrected_barycentric_coordinates = tf.concat(
      per_image_uncorrected_barycentric_coordinates, axis=0)  # (?, 3)
  vertex_ids = tf.concat(per_image_vertex_ids, axis=0)  # (?, 3)
  ...
  corner_attributes = tf.gather(flattened_vertex_attributes, vertex_ids)  # (?, 3, 11)
